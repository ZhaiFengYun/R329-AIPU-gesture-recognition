...
def tf_InvertedResidualWithShift(net, shift_buffer, inp, oup, stride, expand_ratio, n):
    ...
    net1 = tf.slice(net, [0, 0, 0, 0], [-1, -1, -1, n])

    net2 = tf.slice(net, [0, 0, 0, n], [-1, -1, -1, -1])
...
if __name__ == '__main__':
    ...
    #         saver.restore(sess, '/mnt/data/tf_tsm/new_ckpt/model_ckpt')  # 加载到当前环境中
    #
    #         y = sess.run(y)
    #         # x9=sess.run(x9)
    #         #
    ...
    #saver = tf.train.Saver()
    # y_ = tf.placeholder(tf.float32, [None, 27])
    ...
    #
    #     sess.run(tf.global_variables_initializer())
    #     #saver.restore(sess, '/mnt/data/tf_tsm/new_ckpt/model_ckpt')
    ...
    # with tf.gfile.FastGFile('/mnt/data/tf_tsm/' + 'model.pb', mode='wb') as f:
    ...
    with tf.Graph().as_default():
        ...
        with open('/mnt/data/tf_tsm/model3.pb', "rb") as f:
